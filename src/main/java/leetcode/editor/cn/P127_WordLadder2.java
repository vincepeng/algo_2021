...
public class P127_WordLadder2 {
    ...
    public static void main(String[] args) {
        Solution solution2 = new P127_WordLadder2().new Solution();
        ...
        Solution solution = new P127_WordLadder2().new Solution();
    ...
    class Solution {
        ...
        private void doFind(String endWord, LinkedList<String> wordsRecords, HashMap<Integer, Map<String, List<Pair>>> wordIndexMap) {
            ...
            for (String childWord : children) {
                //已经有的就先不遍历;
                Integer level = subStrResult.get(childWord);
                if (level != null && level <= wordsRecords.size() + 1) {
                    continue;
                }
                //                System.out.println(childWord + "  level:" + level + " ,list:" + result.size());
                //下一层表达
                wordsRecords.addLast(childWord);
                doFind(endWord, wordsRecords, wordIndexMap);
                wordsRecords.removeLast();
            }
        }
